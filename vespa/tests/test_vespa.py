...
def test_cannot_connect_is_critical(aggregator):
    ...
    aggregator.assert_service_check(check.METRICS_SERVICE_CHECK, VespaCheck.CRITICAL, count=1)
    aggregator.assert_service_check(check.METRICS_SERVICE_CHECK, VespaCheck.WARNING, count=0)
...
def test_service_reports_down(aggregator):
    ...
    aggregator.assert_service_check(
        check.PROCESS_SERVICE_CHECK,
        VespaCheck.CRITICAL,
        count=1,
        message='Service vespa.down-service reports down: No response',
        tags=['instance:down-service', 'vespaVersion:7.0.0', 'vespa-service:vespa.down-service', 'tag1:val1'],
    )
...
def test_service_reports_unknown(aggregator):
    ...
    aggregator.assert_service_check(
        ...
        tags=['instance:unknown-service', 'vespaVersion:7.0.0', 'vespa-service:vespa.unknown-service', 'tag1:val1'],
    )
    aggregator.assert_service_check(check.PROCESS_SERVICE_CHECK, VespaCheck.OK, count=0)
...
def test_down_service_does_not_raise(aggregator):
    ...
    aggregator.assert_service_check(
        ...
        VespaCheck.OK,
        count=1,
        tags=['instance:up-service', 'vespaVersion:7.0.0', 'vespa-service:vespa.up-service', 'tag1:val1'],
    )
